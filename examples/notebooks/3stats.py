...
class Prior:
    ...
    def logpdf(self, theta: np.ndarray) -> np.ndarray:
        ...
        log_prior = np.full(mu.size, -np.inf)
        log_prior[in_bounds] = -np.log(mu_max - mu_min) - np.log(sigma_max - sigma_min)
        return log_prior
...
def model(theta):
    ...
    mu = theta[:, 0]
    sigma = theta[:, 1]
    result = np.empty((n_batch, 3), dtype=np.float64)
    for i in range(n_batch):
        y_i = np.random.normal(mu[i], sigma[i], size=num_samples)
        result[i] = sum_stats(y_i)
    return result


def f_dist(theta, out=None):
    theta = np.atleast_2d(theta)
    n_batch = theta.shape[0]
    ss = model(theta)
...
ax.plot(it, rho_dif[2, :], label=r"median")
ax.grid(True, alpha=0.3)
...
ax.plot(it, u_dif[2, :], label=r"median")
ax.grid(True, alpha=0.3)
...
# %%
save_sabc_result(out_dif_2, HERE / "test_results" / "out_DE_sing_3stats.pkl")

# -------------------------
# out_dif_2 = load_sabc_result(HERE / "test_results" / "out_DE_sing_3stats.pkl")
# -------------------------
...
# %%
save_sabc_result(out_dif_mult_2, HERE / "test_results" / "out_DE_mult_3stats.pkl")

# -------------------------
# out_dif_mult_2 = load_sabc_result(HERE / "test_results" / "out_DE_mult_3stats.pkl")
# -------------------------
